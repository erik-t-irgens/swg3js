...
- **Facing**: the body faces the camera while fighting, in the air, rolling and during the wall moves, so a flip or a wall run reads the way the move was made; on the ground it turns the way it runs, unless the converted player carries sideways and backwards clips (the converter now asks for any clip named `strafe`, `backward`, `walk_back` or `run_back`), in which case it faces the camera and plays those.
