...
- **Animations**: SWG's own walk, run, swim and idle stay. Jedi Academy's swings, stances (held while standing with the saber drawn), jumps, landings, flips and rolls come in when the player is converted with `--jka=<path to Jedi Academy's GameData or base folder>` (see the converter README); the importer retargets them onto the SWG skeleton. Without them, jumps use SWG's clips and swings fall back to the stand-in arm swing.
