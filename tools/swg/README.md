...
`snapshot` reads the planet's world snapshot (`snapshot/<planet>.ws`), keeps every top-level object within the radius of the centre, resolves each object template through its base-template chain to an appearance (portal buildings use their exterior cell), converts each unique mesh, and writes `layout.json` plus a `layout` category in the manifest. It also copies the planet's terrain template (`terrain/<planet>.trn`) to `terrain.trn` and every building's terrain-modification layer (`terrainModificationFileName`, a `.lay` file) into `terrain/`, recording which object uses which. The game centres the world on that point, mirrors X to match the converted meshes, instances every object with its exact rotation, and gives the larger ones near spawn exact collision.
