 ...
 */
export function buildGallery({ log = () => {}, only = ['houses', 'vehicles', 'weapons', 'anims'], limit = Infinity }, deps) {
  const objects = [];
  const sections = [];
  let z = 0;
  ...
  if (only.includes('weapons')) section('weapons', 'Weapons', deps.templates('object/weapon/'), { gap: 1.5, rowWidth: 120, y: 1.1 });
  if (only.includes('vehicles')) section('vehicles', 'Vehicles', deps.templates('object/mobile/vehicle/'), { gap: 4, rowWidth: 200 });
  if (only.includes('houses')) section('houses', 'Player houses', deps.templates('object/building/player/'), { gap: 8, rowWidth: 400 });
  deps.copySky?.();
