...
import { orbitFor } from './world/sceneBackdrop.ts';
...
// that draws no frames; the knob itself is also on `__debug.fighters({ cover: â€¦ })`.
...
function mountPrompt(v: import('./vehicles/vehicle').Vehicle, wingsKey: string = WINGS_KEY): string {
  const k = v.spec.kind;
  const bar = (f: number) => 'â–®'.repeat(Math.round(f * 8)) + 'â–¯'.repeat(8 - Math.round(f * 8));
  const boost = v.spec.boost === 'heat' ? ` Â· <b>Shift</b> boost Â· heat ${bar(v.meter)}${v.overheated > 0 ? ' BURNT OUT' : ''}` : v.spec.boost === 'burst' ? ` Â· <b>Shift</b> boost ${bar(v.meter)}` : '';
  const hop = v.spec.hop ? ' Â· <b>Space</b> hop' : '';
  const fly = v.spec.fly ? ' Â· look up/down or <b>Space</b>/<b>X</b> to climb and sink' : '';
  if (k === 'ship') {
    // Down on the ground: what gets it up again, and why a put-down was refused.
    if (v.landed) return `${v.space ? 'set down Â· <b>W</b> lifts off along the surface' : 'landed Â· <b>W</b> or <b>Space</b> lifts off'} Â· <b>E</b> leave${v.space ? ' (the boots take hold of what it stands on)' : ''}${v.landNote ? ` Â· ${v.landNote}` : ''}`;
    if (v.holding) return 'setting downâ€¦';
    ...
    const down = SHIP_GROUND.rule === 'landing' ? ` Â· <b>Ctrl</b> brings it down, held at the bottom to set it down Â· <b>${keyName(CUT_ENGINES_KEY)}</b> cuts the engines` : '';
    // Out in space there is no ground: the same key sets the hull down on whatever it has come to a stop over.
    const setDown = v.space && SHIP_GROUND.rule === 'landing' ? (v.setDownNear ? ` Â· <b>${keyName(CUT_ENGINES_KEY)}</b> sets it down on what is under you` : Math.abs(v.speed) <= SPACE_LANDING.speed ? ' Â· nothing under it to set down on' : '') : '';
    const hover = `<b>W</b> throttle up into flight Â· mouse turns Â· <b>Space</b>/<b>Ctrl</b> rise and sink Â· <b>A/D</b> slide${v.space ? setDown : down}`;
    // Stopped in the air the ship holds its height, so the way down belongs on the flight line too.
    const flight = `<b>W</b>/<b>S</b> throttle up and down Â· mouse: in the circle aims the guns, out of it keeps turning the ship Â· <b>A/D</b> roll Â· <b>Space</b>/<b>X</b> pitch${v.powered ? '' : ' Â· <b>ENGINES CUT</b>'}${v.space ? setDown : Math.abs(v.speed) < 2 ? down : ''}`;
    ...
    const wings = v.wings.length ? ` Â· <b>${keyName(wingsKey)}</b> ${v.wings.chosen ? 'close' : 'open'} the wings${v.wings.pilot && !v.wings.target ? ' (they open with room under them)' : ''}` : '';
    return `<b>E</b> leave Â· ${v.airborne ? flight : hover} Â· <b>wheel</b> zoom, all the way in for the cockpit Â· <b>Alt</b> look around${v.guns.length ? ' Â· <b>click</b> fires Â· <b>Tab</b> next target' : ''}${wings} Â· <b>Shift</b> burn Â· ${v.airborne ? 'flying' : 'hovering'} Â· ${Math.round(Math.abs(v.speed) * 3.6)} km/h${v.hp < v.maxHp ? ` Â· hull ${Math.round((v.hp / v.maxHp) * 100)}%` : ''}${v.landNote ? ` Â· ${v.landNote}` : ''}`;
  }
  const turn = k === 'ground' ? 'mouse or <b>A/D</b> turn' : 'mouse or <b>A/D</b> steer';
  const hull = v.hp < v.maxHp ? ` Â· hull ${Math.round((v.hp / v.maxHp) * 100)}%${v.hp / v.maxHp < 0.34 ? ' LIMPING' : v.hp / v.maxHp < 0.67 ? ' smoking' : ''}` : '';
  return `<b>E</b> dismount Â· <b>W/S</b> throttle Â· ${turn} Â· <b>Alt</b> look around${boost}${hop}${fly} Â· ${k} Â· ${Math.round(Math.abs(v.speed) * 3.6)} km/h${hull}`;
}
 ...
 * the bar before the hand reaches the key and slow enough that the asking â€” the lift underfoot, the
 * elevators near, the doorless building near, the nearest vehicle â€” costs an eighth of what it did;
...
interface ShipCrossing {
  ...
  height: number;
  crew?: ShipCrew | null;
  /** Where the ship comes out and which way it faces (game frame), instead of over the spawn at heading Ï€. */
...
class App {
   ...
   * â€” which is every world but one, and every pack converted before the join was written â€” has no
  ...
  constructor(private readonly physics: Physics) {
    ...
    // Where a blow on the player came from. Every striker already passes it â€” a creature, a fighter,
    // a mobile and a bolt all call `damage(amount, from, push, source)` â€” and the record now hands it
    ...
    (window as unknown as { __debug: unknown }).__debug = {
       ...
       * there, so anything is safe to ask for. It is not the speed a ripple is seen to travel at â€”
       ...
       * program differ from another â€” the light signature, which face is drawn, fog, the shadow
       ...
       * is *for* â€” the light set, a side, an alpha test, the cascades' defines, the wet wrap, a
       ...
       * `shimmerRate`, `shimmerReach`, `shimmerSurface`, `shimmerDeep`) â€” every key of
      ...
      underwater: (tune?: Partial<UnderwaterTune>) => {
        ...
        return {
          ...
          // a number goes. It is not an error â€” the rest of the call went through.
          ignored,
          tuning: { ...T },
      ...
      gradeSelfTest: () => {
        const pass = this.gradePass();
        if (!pass) return 'the effects are off (turn Effects on): the self test draws the gradeâ€™s own shader';
       ...
       * the last full second that were not by design â€” the number to look for is 0, and the 4 Hz
      ...
      spawnBox: (w = 1, h = 1, l = 2.4, kind: VehicleKind = 'speederbike') => {
        ...
        return `box ${w}Ã—${h}Ã—${l} spawned as a ${kind} at ${v.pos.toArray().map((n) => n.toFixed(1)).join(',')}`;
      },
      ...
      mobile: async (idOrFind: string, metres = 10, n = 1) => {
        ...
        if (this.player.aboard) return 'nothing can be stood aboard a shipâ€™s rooms';
       ...
       *                                      the same knob as `__debug.fighters({ cover: â€¦ })`)
      ...
      captureShip: (key?: string) => {
        ...
        console.log(line);
        return { place: key, ship: v.def?.id ?? null, pose, metres: Number(camAt.distanceTo(g).toFixed(1)), line, paste: line };
      },
    ...
    this.world.refuseMobiles = () => (this.player?.aboard ? 'nothing can be stood aboard a shipâ€™s rooms' : null);
  ...
  /**
   * You hurt something: the tick on the crosshair, the line in words and â€” with the setting on â€” a
  ...
  private landedHit(target: { pos: THREE.Vector3; halfHeight: number; key: number; label: string }, amount: number, killed: boolean): void {
    ...
    // the living only â€” a shot at a hull or a turret never reaches it â€” so the whole of the rule is
   ...
   * The words the bar shows never cost anything; the *asking* does â€” the lift underfoot, the
   ...
   * eight times a second like the rest, and on a world with no gates at all â€” which is every world
   * but one â€” it is one comparison and nothing more.
   *
   ...
   * it allocates nothing â€” the primitives take numbers and colour indices only.
   */
   ...
   * travel, a jump or an effects switch `play`, which at the select screen â€” where there is no
   * world, no player and nothing playing â€” filed the session's first fourteen programs as built in
  ...
  private jumpPrompt(inLift: boolean): string | null {
    ...
    if (inLift) return 'in hyperspace Â· <b>E</b> lift';
    if (p.piloting) return 'in hyperspace Â· the ship comes out when the way ahead is ready Â· <b>E</b> lets go of the controls';
    if (room.pilotSpot && p.pos.distanceTo(room.pilotSpot) < CONTROLS_RANGE) return 'in hyperspace Â· <b>E</b> take the controls';
    return 'in hyperspace Â· the ship comes out when the way ahead is ready';
  }
   ...
   * pilot. Nothing here is made afresh â€” the two strings are joined only when the target itself
  ...
  private targetHud(pilot: Vehicle): TargetView | null {
    ...
    if (t !== this.targetWordsOf) {
      ...
      tv.kind = c ? `${FACTION_LABEL[c.faction]}${c.type ? ` Â· tier ${c.type.tier}` : ''}` : t.spec.kind;
    }
   ...
   * change; shown again, the dot takes the rule it always had â€” it is the Bounty Hunter's.
   */
  ...
  private worldSpawns(): import('./ui/npcUi').WorldSpawns {
    return {
      ...
      why: () => 'only the worldâ€™s admin may stand creatures here',
      ask: (entry, n) => {
        const cat = this.world.mobileCatalogue;
        const mobiles = this.world.mobiles;
  ...
  async spawnVehicle(def: VehicleDef, kind?: VehicleKind): Promise<string> {
    ...
    this.messages.note(`preparing the ${def.label}â€¦`);
    ...
    try {
      ...
      const size = [b.max[0] - b.min[0], b.max[1] - b.min[1], b.max[2] - b.min[2]].map((n) => n.toFixed(1)).join('Ã—');
   ...
   * it opens on. Deliberately the last thing E can mean and the first thing a fight takes away â€”
  ...
  private async crossTo(from: WalkableRoom, to: CrossTo): Promise<void> {
    ...
    if (!room && to.kind === 'peer') {
      ...
      this.messages.note(`building the rooms of the ${to.label}â€¦`);
  ...
  private async boardPeerShip(id: number): Promise<void> {
    ...
    if (!room) {
      this.messages.note(`building the rooms of the ${rooms.label(id)}â€¦`);
  ...
  run(): void {
    ...
    const step = () => {
      ...
      // kept struct, because the asking is what costs â€” the lift underfoot, the elevators near, the
      // doorless building near and the nearest vehicle â€” and never the words. It is emptied at once
      ...
      // and every one of them â€” the lift underfoot, the elevators near, the doorless building near,
      // the nearest vehicle, the ship in reach of the boots â€” is now read out of the bar's own
      ...
      const shipHint = !full ? '' : this.spaceGate === 'up' ? ` Â· <b>at altitude for space: ${shipKey}</b> ship menu` : this.world.planet.space ? ` Â· <b>${shipKey}</b> ship menu` : '';
      let prompt = '';
      if (full) {
        if (player.noclip) prompt = `<b>NOCLIP</b> ${Math.round(player.noclipSpeed)} m/s Â· <b>WASD</b> fly Â· <b>Space</b> up Â· <b>Ctrl</b> down Â· <b>Shift</b> fast Â· <b>+</b>/<b>-</b> speed Â· <b>N</b> off`;
        ...
        else if (player.piloting) prompt = `at the controls of the ${player.piloting.spec.label} Â· ${player.piloting.landed ? `landed Â· <b>W</b> or <b>Space</b> lifts off` : `<b>W</b>/<b>S</b> throttle Â· mouse steers${player.piloting.spec.ship && SHIP_GROUND.rule === 'landing' ? ` Â· hold <b>Ctrl</b> to set down Â· <b>${keyName(CUT_ENGINES_KEY)}</b> cuts the engines` : ''}`} Â· <b>Alt</b> looks around Â· <b>E</b> lets go Â· ${Math.round(Math.abs(player.piloting.speed) * 3.6)} km/h${shipHint}${player.piloting.landNote ? ` Â· ${player.piloting.landNote}` : ''}`;
        // Standing on something out in space: the boots hold, a jump lets go, and E climbs into a ship beside you.
        else if (isSurfaceRoom(player.aboard)) prompt = `<b>gravity boots</b> on ${S8.bootsReach ? 'a surface Â· <b>E</b> climbs into the ship' : 'a surface Â· <b>E</b> takes them off'} Â· <b>jump</b> lets go${player.aboard.atEdge ? ' Â· <b>the surface underfoot runs out near here</b>' : ''} Â· <b>${shipKey}</b> ship menu`;
        else if (player.aboard) prompt = (player.aboard.pilotSpot && player.pos.distanceTo(player.aboard.pilotSpot) < CONTROLS_RANGE ? `<b>E</b> take the controls` : `aboard ${player.aboard.vehicle.spec.label} Â· <b>E</b> step out`) + (this.world.planet.space ? ` Â· <b>${shipKey}</b> ship menu` : '');
        else if (player.eva) prompt = `adrift Â· <b>W/S</b> thrust ahead and back Â· <b>A/D</b> sideways Â· <b>Space/Ctrl</b> up and down Â· mouse turns Â· <b>Z/V</b> roll Â· <b>${keyName(input.bindings.brake[0] ?? '')}</b> brake Â· ${Math.round(player.vel.length() * 3.6)} km/h${S8.near === 'board' ? ' Â· <b>E</b> board' : S8.near ? ' Â· <b>E</b> mount' : ' Â· <b>E</b> gravity boots'}${performance.now() - this.bootsNoteAt < SURFACE_ROOM.note * 1000 && this.bootsNote ? ` Â· ${this.bootsNote}` : ''}`;
      ...
      // The message line ages on the real clock, not the simulation's â€” `rawDt`, not the step's
      ...
      // changed â€” eight rows of number comparisons, nothing allocated, and nothing written at all
      ...
      const at = player.worldPos;
      // "Nearby" walked every living thing on the planet â€” a few hundred in a busy town â€” on every
      ...
      if (made.made > 0 && phase === 'play' && this.lastPrograms > 0) {
        ...
        const over = made.made > SHADER_TUNE.playBudget ? ' â€” over the one a frame this game holds to' : '';
        console.info(`shaders: ${made.made} built during play${churn} (${stats.frameMs.toFixed(0)} ms frame, ${made.live} programs in all, first "${made.first}")${over} â€” __debug.shaders() for the rest`);
      }
