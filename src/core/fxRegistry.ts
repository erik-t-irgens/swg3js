...
export const FX_KNOBS: readonly FxKnobDef[] = [
  ...
  { key: 'ssao', pass: 'ssao', label: 'Ambient occlusion', kind: 'toggle', requires: ['effects'], hint: 'Corners, creases and the ground under things darken where less of the sky reaches them.' },
  ...
  { key: 'lensFlare', pass: 'lensFlare', label: 'Lens flare', kind: 'toggle', requires: ['effects'], hint: 'Glare and ghosts thrown across the view by a sun or a bright source in front of the camera.' },
  { key: 'lensFlareStrength', pass: 'lensFlare', label: 'Lens flare strength', kind: 'range', min: 0, max: 1.5, step: 0.05, format: two, requires: ['effects', 'lensFlare'], hint: '0.2 a clean lens, 0.5 a film camera, 1.5 a cheap one.' },
  { key: 'colorGrade', pass: 'colorGrade', label: 'Colour grade', kind: 'toggle', requires: ['effects'], hint: 'Each planet’s own white balance, tint and contrast, applied just before tone mapping.' },
  { key: 'colorGradeStrength', pass: 'colorGrade', label: 'Colour grade strength', kind: 'range', min: 0, max: 1, step: 0.05, format: two, requires: ['effects', 'colorGrade'], hint: '0 the raw picture, 1 the planet’s look in full.' },
  ...
  { key: 'filmGrain', pass: 'grainVignette', label: 'Film grain', kind: 'toggle', requires: ['effects'], hint: 'A fine moving grain over the picture. Off by default: at 144 frames a second it reads as noise.' },
  { key: 'filmGrainStrength', pass: 'grainVignette', label: 'Film grain strength', kind: 'range', min: 0, max: 1, step: 0.05, format: two, requires: ['effects', 'filmGrain'], hint: '0.2 barely there, 0.6 a film stock.' },
  { key: 'vignette', pass: 'grainVignette', label: 'Vignette', kind: 'toggle', requires: ['effects'], hint: 'The corners of the picture fall off a little, as a lens does.' },
  { key: 'vignetteStrength', pass: 'grainVignette', label: 'Vignette strength', kind: 'range', min: 0, max: 1, step: 0.05, format: two, requires: ['effects', 'vignette'], hint: '0.15 a hint, 0.25 a lens, 0.6 a telescope.' },
];
...
export const FX_PASSES: readonly FxPassDef[] = [
  ...
  { id: 'lensFlare', stage: 'lens', toggles: ['lensFlare'], required: false, needs: ['linearDepthHalf'], budgetMs: 0.12, typical: true, canBeLast: false, live: false, why: 'Reflections inside the lens. After bloom, so the flare is not itself thresholded and bloomed and so it can reuse the bloom’s blurred levels; before the grade and the tone curve so it is graded with the picture.' },
  { id: 'colorGrade', stage: 'lens', toggles: ['colorGrade'], required: false, needs: [], budgetMs: 0.04, typical: true, canBeLast: false, live: false, why: 'Each planet’s white balance, tint, saturation and contrast in the linear high range, just before the tone curve, so the highlights roll off after grading rather than clipping.' },
  ...
  { id: 'grainVignette', stage: 'display', toggles: ['filmGrain', 'vignette'], required: false, needs: [], budgetMs: 0.1, typical: true, canBeLast: true, live: false, why: 'Last: grain over final display values keeps its look whatever the exposure, and the vignette falls off predictably in display space.' },
