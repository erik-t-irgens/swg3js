...
export class Gallery {
  ...
  private wake(s: Slot): void {
    ...
    if (s.clip.joints?.length || isAdditive(clip.name)) {
      ...
      // Joint names case-insensitively: the animation files spell them their own way.
      const own = new Set((s.clip.joints ?? []).map((j) => j.toLowerCase()));
      const nodeOf = (t: THREE.KeyframeTrack) => (THREE.PropertyBinding.parseTrackName(t.name).nodeName ?? '').toLowerCase();
