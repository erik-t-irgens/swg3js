...
// Messages out: { type: 'ready', info } | { type: 'grid', id, heights, shaders, excluded, floraCollidable, floraNonCollidable } | { type: 'error', message }
...
ctx.onmessage = (e: MessageEvent<InitMessage | GenerateMessage>) => {
  ...
  try {
    if (msg.type === 'init') {
      ...
      ctx.postMessage({ type: 'grid', id: msg.id, heights: g.heights, shaders: g.shaders, excluded: g.excluded, floraCollidable: g.floraCollidable, floraNonCollidable: g.floraNonCollidable }, [g.heights.buffer, g.shaders.buffer, g.excluded.buffer, g.floraCollidable.buffer, g.floraNonCollidable.buffer]);
    }
  } catch (err) {
