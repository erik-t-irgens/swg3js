...
export class WardrobeUi {
  ...
  async attach(character: Character, baseUrl: string): Promise<void> {
    ...
    try {
      ...
      this.body.innerHTML = '<div class="wardrobe-empty">No converted wardrobe for this character. Run <code>npm run swg -- wardrobe &lt;swg-dir&gt; assets-private</code>.</div>';
      return;
    }
    this.build();
  }
  ...
  private build(): void {
    ...
    for (const sel of this.body.querySelectorAll<HTMLSelectElement>('select')) {
